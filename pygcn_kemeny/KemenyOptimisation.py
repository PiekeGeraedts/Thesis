...
for n in range(N):
	...
	if n!= 0:
		exp_mvg_avg1.append(beta1*exp_mvg_avg1[n-1] + (1-beta1)*SPSA[n])
	else: 
		exp_mvg_avg1.append(SPSA[n])
...
#print (V)
#print (adj.values())
torch.save({'optimizer_state_dict': optimizer.state_dict(), 'epoch':args.epochs}, save_path+'optimiser.pt')
torch.save({'V_norm': V_norm, 'nrm_mthd': name, 'indices': adj._indices(), 'values': adj._values(), 'size': adj.size(), 'features': features, 'labels': labels, 'idx_train': idx_train, 'idx_val': idx_val, 'idx_test': idx_test}, save_path +'data.pt')
