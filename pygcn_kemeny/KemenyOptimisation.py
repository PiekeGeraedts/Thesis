...
gamma = args.gamma	
normalise = subtract_normalisation 	#NOTE: paper_normalisation does not work yet, though it seemed so because it is 'close'
...
for n in range(N):
	...
	with torch.no_grad():
		...
		normalized0, normalized1 = normalise(indices, V0, size, eps), normalise(indices, V1, size, eps)
		try:
			K, K0, K1 = Kemeny(indices, V, size), Kemeny(indices, normalized0, size), Kemeny(indices, normalized1, size)
...
save_path = f'KemenyOptimisation/lr={args.lr}_'
print (save_path)
