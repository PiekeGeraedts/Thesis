...
from utils import load_data, accuracy, Kemeny, Kemeny_spsa, normalisation1, constraint
...
parser.add_argument('--mu', type=float, default=0.1,  
                    help='Restriction on change in V')
...
def train(epoch):
    ...
    optimizer.zero_grad()
    with torch.no_grad():
        model.edge_weights = constraint(model.edge_weights, initial_edge_weights, mu)
    ...
    with torch.no_grad():
        ...
        '''
        K_lst.append(Kemeny(adj_indices, model.edge_weights, adj_size))
  
    loss_train = gcn_loss #- gamma*sum(model.edge_weights * ((kemeny_loss0 - kemeny_loss1)/2*perturbation))
    acc_train = accuracy(output[idx_train], labels[idx_train])  
    loss_train.backward()
    model.edge_weights.grad = torch.add(model.edge_weights.grad, Kemeny_spsa(adj_indices, model.edge_weights, adj_size, eta), alpha=gamma)
    optimizer.step()
    ...
    gradspsa_lst.append(torch.norm(torch.add (model.edge_weights.grad, Kemeny_spsa(adj_indices, model.edge_weights, adj_size, eta), alpha=gamma)))
