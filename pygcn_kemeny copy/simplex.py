...
def toSpherical(mx, eps=0):
    """the given mx is a prob. vector, return spherical vector."""
    n = vec.shape[0]
    sph = torch.zeros(n-1)
    mx = torch.sqrt(mx)
    for i in range(n-1):
        if (np.equal(vec[i:], np.zeros(n-i))):
            break #All angles remain at zero. Using below formulas will give division by 0
        if (i == n-2):  
            if (vec[i] >= 0): 
                sph[i] = np.acos(vec[i]/euclidean(vec[i:], torch.zeros(n-i)))   
            else:
                sph[i] = 2*torch.pi - np.arccos(vec[i]/euclidean(vec[i:], np.zeros(n-i)))   
        else:
...
normalise = squared
random_mx = np.random.randn(N,n)
random_sph = np.random.randn(N,n-1)
...
for i in range(int(N/3)):
    ...
    Knrm_paper[i] = sum_norm(SPSA_kem(V, paper))
    #print ('=sft=')
    #print (data_sft)
    #print (Vnrm_sft[i])
    #print (Knrm_sft[i])
    ...
    #Knrm_sub[i] = sum_norm(SPSA_kem_sph(sph))
    #print ('=sph=')
    #print (data_sph)
    #print (Vnrm_sph[i])
    #print (Knrm_sph[i])
