...
from utils import load_data, accuracy, WeightClipper, Kemeny, Kemeny_spsa, subtract_normalisation, squared_normalisation, softmax_normalisation, paper_normalisation
#from pygcn_kemeny.models import GCN
from tools import AdjToSph, SphToAdj, KemenySpherical, plot_grad_flow
...
def update_print(epoch, acc_val, acc_train, loss_val, loss_train, t):
    ...
    n_eps = torch.where(V <= 1.5*eps)[0].shape[0]
 ...
 #   grad_lst.append(torch.norm(model.edge_weights.grad))
    edgeweights_lst.append(torch.norm(model.edge_weights.grad))
...
path = f'Results/2021/Cora/performanceGCNwithV/{args.lrV}_{name}_'
print (path)
...
#fig.savefig(path + 'edgeweights.jpg')
...
fig, ax = plt.subplots()
ax.plot(n_eps_lst)
ax.set_title('V_ij<eps')
fig.savefig(path + 'neps.jpg')
plt.show()
# log information
eta_info = f"Eta is chosen fixed at {eta}"
log = np.array([str(date.today()), "nlayers:"+str(args.nlayer), "epochs:"+str(args.epochs)+"+"+str(args.N), "clipper:"+str(args.clipper), "lr:"+lr_info, "gamma:"+str(gamma), "eps:"+str(eps), eta_info, "projection:" + str(args.projection), "normalisation method:" + str(name), "GCN loss test:" + str(GCN_loss_test), "GCN acc_test:" + str(GCN_acc_test), "loss test:" + str(loss_test), "acc_test:" + str(acc_test)])
